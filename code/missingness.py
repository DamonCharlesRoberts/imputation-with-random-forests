...
sim1comp = pd.DataFrame(sim1, columns = ['ColumnA', 'ColumnB', 'ColumnC'])
...
class sim_missingness:
	def __init__(sim, types):
		if types == 'MCAR':
			sim = produce_NA(sim1, p_miss = 0.4, mecha = types)
		elif types == 'MAR':
			sim = produce_NA(sim1, p_miss = 0.4, mecha = types, p_obs = 0.5)
		elif types == 'MNAR':
			sim = produce_NA(sim1, p_miss = 0.4, mecha = types, p_obs = 0.5, opt = 'logistic')
		else:
			print('Invalid Argument')
		sim = sim['X_incomp']
		sim = pd.DataFrame(sim.numpy(), columns = ['ColumnA', 'ColumnB', 'ColumnC'])
...
sim10comp = pd.DataFrame(sim10, columns = ['ColumnA', 'ColumnB', 'ColumnC', 'ColumnD', 'ColumnE', 'ColumnF', 'ColumnG', 'ColumnH', 'ColumnI', 'ColumnJ'])
...
class sim_missingness:
	def __init__(sim, types):
		if types == 'MCAR':
			sim = produce_NA(sim10, p_miss = 0.4, mecha = types)
		elif types == 'MAR':
			sim = produce_NA(sim10, p_miss = 0.4, mecha = types, p_obs = 0.5)
		elif types == 'MNAR':
			sim = produce_NA(sim10, p_miss = 0.4, mecha = types, p_obs = 0.5, opt = 'logistic')
		else:
			print('Invalid Argument')
		sim = sim['X_incomp']
		sim = pd.DataFrame(sim.numpy(), columns = ['ColumnA', 'ColumnB', 'ColumnC', 'ColumnD', 'ColumnE', 'ColumnF', 'ColumnG', 'ColumnH', 'ColumnI', 'ColumnJ'])
